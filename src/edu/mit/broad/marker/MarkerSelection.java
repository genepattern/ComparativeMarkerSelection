...
public class MarkerSelection {
    ...
    public MarkerSelection(ExpressionData _dataset, String _datasetFile, ClassVector _classVector, String _clsFile,
        ...
        if (_complete && _smoothPValues) {
            System.out
                    .println(
                            "Smooth p-values set to false. Smoothing p-values disabled when performing all possible permutations.");
            this.smoothPValues = false;
        }
        if (significanceBooster && !smoothPValues) {
            System.out
                    .println(
                            "Smooth p-values set to true. Smooth p-values must be true when using significance booster.");
            ...
            if (performAllPairs) {
                ...
                for (int i = 0; i < n; i++) {
                    for (int j = i + 1; j < n; j++) {
                        ...
                        String tempOutputFileName = baseOutputFileName + "." + classVector.getClassName(i) + ".vs." +
                                classVector.getClassName(j) + ".odf";
                        new MarkerSelection(pairwiseDataset, datasetFile, allPairs[allPairsIndex++], clsFile,
                                _numPermutations, testDirection, tempOutputFileName, balanced, complete, metric, minStd,
                                seed, pairwiseConfounder, confoundingClsFile, removeFeatures, theta, smoothPValues,
                                gamma);
    ...
    final void computePValues() {
        ...
        if (numPermutations <= 0) {
            ...
            BigInteger totalPermutations = ((BalancedCompletePermuter) (permuter))
                    .getTotal();
            if ((totalPermutations.compareTo(new java.math.BigInteger("" + Integer.MAX_VALUE))) == 1) {
                AnalysisUtil.exit("Number of permutations exceeds maximum of " + Integer.MAX_VALUE + ".");
            }
            ...
            if (balanced) {
                BigInteger totalPermutations =
                        new BalancedCompletePermuter(classZeroIndices, classOneIndices).getTotal();
                ...
                BigInteger totalPermutations =
                        new UnbalancedCompletePermuter(classVector.size(), classZeroIndices.length).getTotal();
                if (new BigInteger("" + numPermutations).compareTo(totalPermutations) == 1) {
                    permuter = new UnbalancedCompletePermuter(classVector.size(), classZeroIndices.length);
                    complete = true;
                    seedUsed = false;
        ...
        for (int i = 0; i < numFeatures; i++) {
            ...
            classZeroStd[i] =
                    org.genepattern.stats.Util.standardDeviation(dataArray, classZeroIndices, i, meanClassZero);
            ...
            classOneStd[i] = org.genepattern.stats.Util.standardDeviation(dataArray, classOneIndices, i, meanClassOne);
        ...
        if (!significanceBooster) {
            for (int i = 1; i < numFeatures; i++) { // ensure maxT is monotonic
                maxT[descendingAbsIndices[i]] =
                        Math.max(maxT[descendingAbsIndices[i]], maxT[descendingAbsIndices[i - 1]]);
            }
        }
        int[] _ranks = null;
        ...
        try {
            ...
            if (!significanceBooster) {
                ...
                if (rowDescriptions != null) {
                    columnNames = new String[]{"Rank", "Feature", "Description", "Score", "Feature P", "Feature P Low",
                            "Feature P High", "FDR(BH)", "Q Value", "Bonferroni", "Fold Change", classZero + " Mean",
                            " Std", classOne + " Mean", classOne + " Std", "Permutations", "Active", "k"};
                    columnTypes = new String[]{"int", "String", "String", "double", "double", "double", "double",
                            "double", "double", "double", "double", "double", "double", "double", "double", "int",
                            "boolean", "int"};
                } else {
    ...
    final int[] nextPermutation() {
        try {
            ...
            AnalysisUtil
                    .exit("An error occurred while reading the permutations file.", e);
            return null;
        }
